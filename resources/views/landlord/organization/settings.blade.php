<x-app-layout>
    ...
    <div class="max-w-2xl space-y-8">
        ...
        <div class="overflow-hidden rounded-xl bg-white shadow-sm ring-1 ring-gray-900/5">
            ...
            <form method="POST" action="{{ route('landlord.settings.update') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                @csrf
                @method('PATCH')
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $org->name) }}" required
                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 transition">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-3">
                            <input type="color" name="primary_color" value="{{ old('primary_color', $org->primary_color) }}"
                                class="h-10 w-12 rounded-lg border border-gray-300 cursor-pointer">
                            <input type="text" id="primary_color_text" value="{{ old('primary_color', $org->primary_color) }}"
                                class="flex-1 rounded-lg border border-gray-300 px-3 py-2.5 text-sm font-mono" readonly>
                        </div>
                    </div>
                    <div>
                ...
                <div>
                    ...
                    <input type="file" name="logo" accept="image/*"
                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm text-gray-700 file:mr-4 file:rounded-md file:border-0 file:bg-indigo-50 file:px-4 file:py-2 file:text-sm file:font-medium file:text-indigo-700 hover:file:bg-indigo-100">
